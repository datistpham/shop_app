...
import { API_URL, HOST, PORT } from "../../config";
...
const Checkout = () => {
  ...
  async function payment() {
    if (chooseMethod === 0) {
      return Alert.alert("Thông báo", "Vui lòng chọn phương thức thanh toán", [
        { text: "Ok" },
      ]);
    }
    if (
      parseInt(chooseMethod) === 2 &&
      (!momo?.[0]?.momo_account || !momo?.[0]?.momo_name)
    ) {
      return Alert.alert(
        "Thông báo",
        "Thông tin momo không hợp lệ hoặc không đầy đủ",
        [{ text: "Ok" }]
      );
    }
    if (
      parseInt(chooseMethod) === 3 &&
      ...
      return Alert.alert(
        "Thông báo",
        "Thông tin ngân hàng không hợp lệ hoặc không đầy đủ",
        [{ text: "Ok" }]
      );
    }
    if (parseInt(chooseMethod) === 2) {
      const resMomo = await axios({
        url: "https://itchy-dirndl-frog.cyclic.app/payment-momo",
        method: "POST",
        data: {
          amount:
            _.sumBy(route.params?.cart, function (e) {
              if (parseInt(e.discount) > 0) {
                return (
                  parseInt(e.amount) *
                  parseInt(e.price) *
                  (1 - parseInt(e.discount) / 100)
                );
              } else {
      ...
      try {
        ...
        await Promise.all(arr_promise).then(function (values) {
          setChange((prev) => !prev);
        ...
        <>
          ...
          >
            ...
            {userName.length > 0 && userName.length < 6 && (
              <View>
                <Text style={{ color: "red" }}>
                  {"Tên người dùng phải ít nhất 6 ký tự"}
                </Text>
              </View>
            )}
            <View
            ...
            {address.length > 0 && address.length < 20 && (
              <View>
                <Text style={{ color: "red" }}>
                  {"Địa chỉ phải có ít nhất 20 ký tự"}
                </Text>
              </View>
            )}
            <View>
              <ComponentInputFillInfo
                value={note}
                setValue={setNote}
                placeholder={"Ví dụ: Chuyển hàng ngoài giờ hành chính"}
              />
            </View>
          </View>
          <View
            ...
            <Button
              ...
              onPress={() => {
                if (
                  ...
                  Alert.alert(
                    "Thông báo",
                    "Thông tin không đẩy đủ. Vui lòng thử lại",
                    [{ text: "Ok" }]
                  );
                } else {
                  setToggle(() => true);
      ...
      {toggle === true && (
        <>
          <TouchableHighlight
            ...
            onPress={() => {
              setChooseMethod(() => 1);
              setMethodName("Giao hàng tận nơi");
            }}
          >
            <View
              style={{
                ...
                borderWidth: 1,
                borderStyle: "solid",
                borderColor: parseInt(chooseMethod) === 1 ? "#2e89ff" : "#fff",
              }}
            >
              <Text
          ...
          >
            <View
              style={{
                ...
                borderWidth: 1,
                borderStyle: "solid",
                borderColor: parseInt(chooseMethod) === 2 ? "#2e89ff" : "#fff",
              }}
            >
              <Text
                style={{ fontSize: 18, fontWeight: "600", marginBottom: 12 }}
              ...
              <Text
                style={{ fontSize: 18, fontWeight: "600", marginBottom: 12 }}
              >
                Số tài khoản: {momo?.[0]?.momo_account || "_"}
              </Text>
              <Text
                style={{ fontSize: 18, fontWeight: "600", marginBottom: 12 }}
              >
                Tên tài khoản: {momo?.[0]?.momo_name || "_"}
              </Text>
            </View>
          </TouchableHighlight>
          {/*  */}
          ...
          >
            <View
              style={{
                ...
                borderWidth: 1,
                borderStyle: "solid",
                borderColor: parseInt(chooseMethod) === 3 ? "#2e89ff" : "#fff",
              }}
            >
              <Text
                style={{ fontSize: 18, fontWeight: "600", marginBottom: 12 }}
              ...
              <Text style={{ marginBottom: 12 }}>
                Sử dụng thẻ ATM hoặc dịch vụ Internet Banking để tiến hành
              ...
              >
                <Text style={{ marginBottom: 12 }}>
                  Cảm ơn bạn đã đặt hàng. Đây là số tài khoản của bạn
                </Text>
                <Text style={{ marginBottom: 12 }}>
                  Số tài khoản:{" "}
          ...
          <View style={{ marginTop: 12 }}>
            <Text style={{ marginBottom: 12 }}>
              Bạn muốn thay đổi thông tin ngân hàng ?
            </Text>
            <Button
              title={"Thay đổi ngân hàng"}
              onPress={() => navigation.navigate("Me", { backButton: true })}
            />
          </View>
        </>
      )}
    ...
    <View style={{ width: "100%" }}>
      <TextInput
        ...
        style={{
          ...
          width: "100%",
          height: 40,
          borderColor: isFocused ? "#2e89ff" : "#e7e7e7",
        }}
        onFocus={handleFocus}
        onBlur={handleBlur}
        value={props?.value}
